"""The database download path.

What these methods do with a `302` and with a body too large to hold is the whole point of
them, so the double here serves both legs: the API's redirect on one host and the object
storage it points at on another. That is what lets a test assert where the API key went,
...
BLOB = f"{STORAGE}/blob?X-Amz-Signature=deadbeef"

SMALL = b"first_ip,last_ip\n10.0.0.0,10.255.255.255\n"
...
class Blob:
    ...
    synthetic, and `delivered` under `size` promises a whole database and cuts it short,
...
class Origin:
    ...
    Both legs go through the same transport, so one record holds the request that carries
    the key and the one that must not.
    """
...
def origin(make_client: ClientFactory, blob: Blob | None = None, retries: int = 2) -> Origin:
    ...
    served.client = make_client(
        api_key=API_KEY, base_url=API, transport=served.transport, retries=retries
    )
    return served
...
def test_download_follows_the_redirect_and_writes_the_file(
    ...
    written = served.client.database.download("bogon_ip_v1", "csvgz", path)
    ...
    assert served.paths() == [DOWNLOAD_PATH, "/blob"]
...
def test_download_bytes_agrees_with_the_streamed_copy(
    ...
    served.client.database.download("bogon_ip_v1", "csvgz", path)
    held = served.client.database.download_bytes("bogon_ip_v1", "csvgz")

    assert held == path.read_bytes(), "the two ways of fetching one file disagree"
    assert held == SMALL
...
def test_download_url_hands_back_the_link_without_fetching_it(
    ...
    assert served.client.database.download_url("bogon_ip_v1", "csvgz") == BLOB
    assert served.paths() == [DOWNLOAD_PATH], "the link was fetched as well as returned"


# The link is the whole answer for this method, and it is only useful because it carries
# no credential of ours: it authorizes itself, so it can be handed to anything that speaks
# HTTP without also handing over the key.
def test_the_returned_link_carries_a_signature_and_no_api_key(
    make_client: ClientFactory,
) -> None:
    served = origin(make_client)

    url = served.client.database.download_url("bogon_ip_v1", "csvgz")

    assert API_KEY not in url, "the API key ended up in a link meant to be passed around"
    assert "X-Amz-Signature" in url, "the link carries no signature, so it authorizes nothing"
...
# no business seeing it - and object storage answers 400 to a request carrying both.
def test_the_api_key_reaches_the_api_and_never_object_storage(
    make_client: ClientFactory, tmp_path: Path
) -> None:
    served = origin(make_client)

    served.client.database.download("bogon_ip_v1", "csvgz", tmp_path / "keys.csv.gz")

    api = served.request(DOWNLOAD_PATH)
...
def test_object_storage_refusing_the_link_is_not_reported_as_an_api_failure(
    make_client: ClientFactory,
) -> None:
    served = origin(make_client, Blob(status=403))

    with pytest.raises(InternetDataError) as caught:
        served.client.database.download_bytes("bogon_ip_v1", "csvgz")
...
# whole database. A transfer can end short without any error reaching the client, so what
...
def test_a_short_transfer_fails_loudly_and_leaves_nothing_behind(
    ...
    path = tmp_path / "half-a-database.csv.gz"
...
# The half of the .part guard a cleanup step cannot fake: a destination opened directly is
# truncated before the first byte arrives, so yesterday's good copy is gone whether or not
# the refresh then succeeds.
...
def test_a_large_body_is_streamed_rather_than_held(
    ...
    path = tmp_path / "vpn_ip_v1.mmdb"

    tracemalloc.start()
    try:
        written = served.client.database.download("vpn_ip_v1", "mmdb", path)
...
def test_a_content_encoded_body_is_not_mistaken_for_a_short_one(
    ...
    assert served.client.database.download_bytes("bogon_ip_v1", "csvgz") == SMALL
...
def test_a_storage_5xx_before_the_body_is_retried(
    ...
    try:
        outcome = served.client.database.download_bytes("bogon_ip_v1", "csvgz")
...
def test_a_transfer_that_dies_part_way_is_not_fetched_again(
    ...
    try:
        getattr(served.client.database, method)("bogon_ip_v1", "csvgz", *rest)
    except (InternetDataError, httpx.HTTPError) as exc:
        failure = exc
